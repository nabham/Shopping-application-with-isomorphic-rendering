...
class Header extends React.Component{
    ...
    componentDidMount(){
        if(sessionStorage.getItem('userName')){
            if(Object.keys(this.props.UserReducer).length==0){
                let userdata = AjaxHelper.reloaddata('/api/reload', {userId:sessionStorage.getItem('userName')});
                this.props.onReload(userdata);
            }
        }
        else{
            const loc = this.props.location.pathname;
            if(loc!='/login' && loc!='/cart' && loc!='/signup' && !loc.includes("productdesc") && !loc.includes("searchPage")){
    ...
    render(){
        ...
        if(loggedIn && Object.keys(this.props.UserReducer).length>0){
            ...
            rows.push(<span key={'nav2'}><Link to="/login" style={{ textDecoration: 'none' }}><li><span><Glyphicon glyph="log-in" /></span> Login</li></Link><Link to="/signup" style={{ textDecoration: 'none' }}><li><span><Glyphicon glyph="user" /></span> Signup</li></Link></span>)
        }
